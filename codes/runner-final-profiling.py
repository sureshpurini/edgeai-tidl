...
def mask_image_opencv(image, bounding_boxes):
    ...
    for xmin, ymin, xmax, ymax in bounding_boxes:
        xmin, ymin, xmax, ymax = map(int, [round(xmin), round(ymin), round(xmax), round(ymax)])
...
def run_fcnn(sess,image,img_width=100, img_height=32):
    ...
    # print("fcn pre process (ms)",time.time()-st)
    st = time.perf_counter()
    ...
    # print("fcn post process(ms)",time.time()-st)
    # print(predicted)
    return predicted
...
def run_yolo_text(sess, image, fcnn_sess, original_image, yolo_bounds):
    ...
    img = np.expand_dims(np.array(img,dtype=np.float32), axis=0).transpose((0, 3, 1, 2))
...
def run_yolo(sess, image, floating_model, shape, yolo_input_name):
    ...
    # preprocess
    st = time.perf_counter()
    img_resized = cv2.resize(image, (shape[3], shape[2]))
